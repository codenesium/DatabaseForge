...
namespace DatabaseContracts.IntegrationTests
{
    public class MSSQLInterfaceTest
    {
        ...
        public void GetDatabaseStructure_SqlServerTypes_ContainsAllCorrectly(string connection)
        {
            ...
            Table typesDatabaseTable = container.Schemas[0].Tables.Find(x => x.Name.ToLower() == "typesdatabase");
            typesDatabaseTable.Should().NotBeNull();
            typesDatabaseTable.Columns.Count.Should().Be(34);
            List<Column> tableColumns = typesDatabaseTable.Columns.ToList();
        ...
        public void GetDatabaseStructure_ForeignKeysBetweenTwoTables_ResponseIsCorrect(string connection)
        {
            ...
            Table tableA = container.Schemas[0].Tables.Find(x => x.Name.ToLower() == "tablea");

            Table tableB = container.Schemas[0].Tables.Find(x => x.Name.ToLower() == "tableb");

            ForeignKey foreignKey = container.Schemas[0].ForeignKeys.Find(x => x.ForeignKeyName.ToLower() == "fk_tableb_tableaid_tablea_id");

            tableA.Should().NotBeNull();

            tableB.Should().NotBeNull();

            foreignKey.Should().NotBeNull();

            foreignKey.Columns.Should().Contain(x => x.PrimaryKeyTableName.ToLower() == "tablea"
                && x.PrimaryKeyColumnName.ToLower() == "id"
                && x.ForeignKeyTableName.ToLower() == "tableb"
                && x.ForeignKeyColumnName.ToLower() == "tableaid");
        }
        ...
        public void GetDatabaseStructure_ForeignKeyToItself_ResponseIsCorrect(string connection)
        {
            ...
            ForeignKey foreignKeyToItself= container.Schemas[0].ForeignKeys.Find(f => f.ForeignKeyName.ToLower() == "fk_users_usersid_users_id");

            foreignKeyToItself.Should().NotBeNull();

            foreignKeyToItself.Columns.Should().Contain(x=>x.PrimaryKeyTableName.ToLower()=="users"
                && x.PrimaryKeyColumnName.ToLower()=="id"
                && x.ForeignKeyTableName.ToLower()=="users"
                && x.ForeignKeyColumnName.ToLower()=="usersid");
        }


        [Theory]
        [InlineData(@"Data Source=CADU-PC\sql2012;Initial Catalog=ForgeDB;Integrated Security=True")]
        public void GetDatabaseStructure_ComplexPrimaryKey_ResponseIsCorrect(string connection)
        {
            // Given
            Codenesium.DatabaseContracts.MSSQLInterface dbInterface = new Codenesium.DatabaseContracts.MSSQLInterface();
            dbInterface.SetConnectionString(connection);
            ...
            Table complexKeyTable = container.Schemas[0].Tables.Find(x => x.Name.ToLower() == "complexkeytable");

            complexKeyTable.Should().NotBeNull();

            complexKeyTable.Constraints.Should().Contain(x => x.Name.ToLower() == "pk_complexkeytable"
                    && x.Columns.Count > 1 && x.IsPrimaryKey && x.IsUnique && x.ConstraintType.ToLower() == "clustered");

        }

        [Theory]
        [InlineData(@"Data Source=CADU-PC\sql2012;Initial Catalog=ForgeDB;Integrated Security=True")]
        public void GetDatabaseStructure_ComplexForeignKeyBetweenTwoTables_ResponseIsCorrect(string connection)
        {
            // Given
            Codenesium.DatabaseContracts.MSSQLInterface dbInterface = new Codenesium.DatabaseContracts.MSSQLInterface();
            dbInterface.SetConnectionString(connection);
            ...
            Table tableAX = container.Schemas[0].Tables.Find(x => x.Name.ToLower() == "tableax");

            Table tableBX = container.Schemas[0].Tables.Find(x => x.Name.ToLower() == "tablebx");

            tableAX.Should().NotBeNull();

            tableBX.Should().NotBeNull();

            tableAX.Constraints.Should().Contain(x => x.Columns.Count > 1 && x.IsUnique && x.IsPrimaryKey && x.ConstraintType.ToLower() == "clustered");

            ForeignKey complexForeignKey = container.Schemas[0].ForeignKeys.Find(x => x.ForeignKeyName.ToLower() == "fk_tablebx_tableax");

            complexForeignKey.Should().NotBeNull();            

            complexForeignKey.Columns.Count.Should().BeGreaterThan(1);

            complexForeignKey.Columns.Should().Contain(x => x.ForeignKeyColumnName.ToLower() == "tableaxid1"
                    && x.PrimaryKeyColumnName.ToLower() == "id1"
                    && x.PrimaryKeyTableName.ToLower()=="tableax"
                    && x.ForeignKeyTableName.ToLower()=="tablebx");

            complexForeignKey.Columns.Should().Contain(x => x.ForeignKeyColumnName.ToLower() == "tableaxid2"
                    && x.PrimaryKeyColumnName.ToLower() == "id2"
                    && x.PrimaryKeyTableName.ToLower() == "tableax"
                    && x.ForeignKeyTableName.ToLower() == "tablebx");
        }
        ...
        [Theory]
        [InlineData(@"Data Source=CADU-PC\sql2012;Initial Catalog=ForgeDB;Integrated Security=True")]
        public void GetDatabaseStructure_TableWithNoPrimaryKey_ResponseIsCorrect(string connection)
        {
            // Given
            Codenesium.DatabaseContracts.MSSQLInterface dbInterface = new Codenesium.DatabaseContracts.MSSQLInterface();
            dbInterface.SetConnectionString(connection);
            ...
            Table noPrimaryKeyTable = container.Schemas[0].Tables.Find(x => x.Name.ToLower() == "noprimarykeytable");

            noPrimaryKeyTable.Should().NotBeNull();

            noPrimaryKeyTable.Constraints.Should().NotContain(x => x.IsPrimaryKey == true);
        }

        [Theory]
        [InlineData(@"Data Source=CADU-PC\sql2012;Initial Catalog=ForgeDB;Integrated Security=True")]
        public void GetDatabaseStructure_View_ResponseIsCorrect(string connection)
        {
            // Given
            Codenesium.DatabaseContracts.MSSQLInterface dbInterface = new Codenesium.DatabaseContracts.MSSQLInterface();
            dbInterface.SetConnectionString(connection);
            ...
            Table testView = container.Schemas[0].Tables.Find(x => x.Name.ToLower() == "testview" && x.IsView);
